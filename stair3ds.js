/**
 * Created by ZJ on 2017/8/8 REVISION: 1
 */
var _Math = {
...
JSQEXBasicStructure.JSQEX_Stair.prototype.JSQEX_changeMaterial = function(index,_url,operation) {//材质修改index材质索引： 0：台阶上下面 1：台阶前后左右 2：支撑梁上下面 3：支撑梁前后左右面 4：侧弦上下面  5：侧弦前后左右面  :6：平台上下面 7：平台前后左右面
    if(index==undefined){
        return;
    }
...
JSQEXBasicStructure.JSQEX_Stair.prototype.JSQEX_AddStair = function(json){//在场景中添加楼梯对象
    ...
    if(this.JSQEX_stairsMesh==null){
        ...
        scene.remove(this.JSQEX_stairsMesh);
    }
    ...
    scene.add(this.JSQEX_stairsMesh);
}

JSQEXBasicStructure.JSQEX_Stair.prototype.JSQEX_initEdit_interface = function(lx,addgui) {//初始化楼梯对象
    var _lx=lx!=undefined?lx:1;
    if(_lx==1){//螺旋
    ...
    this.JSQEX_obj.JSQEX_showgui();//显示楼梯对象的编辑区域
}

JSQEXBasicStructure.JSQEX_Stair.prototype.JSQEX_getRailingpoints = function(_json){
    var arr1=[],arr2=[];
    var arr=this.JSQEX_vertices;
    var pdindex=12;
    if(this.JSQEX_editable_parameter.addgeo.support_beam){
        pdindex+=8;
    }
    if(this.JSQEX_editable_parameter.addgeo.side_string){
        pdindex+=16;
    }
    for(var i=0;i<arr.length;i+=pdindex){
       arr1.push(arr[i].clone().add(arr[i+2]).multiplyScalar(.5))
       arr2.push(arr[i+1].clone().add(arr[i+3]).multiplyScalar(.5))
    }
    for(var j=0;j<arr2.length;j++){
        var geometry = new THREE.BoxGeometry( 100, 100, 100 );
        var material = new THREE.MeshBasicMaterial( {color: 0x00ff00} );
        var cube = new THREE.Mesh( geometry, material );
        cube.position.copy(arr2[j])
        scene.add( cube );
    }
    console.log(arr1);
    console.log(arr2);
...
JSQEXBasicStructure.JSQEX_Stair_Beeline = function(a) {
    ...
    this.layout={length:0,width:1};//布局 length:长度,width:宽度
...
JSQEXBasicStructure.JSQEX_Stair_Beeline.prototype.JSQEX_showgui = function(json) {
    ...
    folderCamera.add( params, '长度',1, 10000, 1).onChange( function(val) {
       ...
       scope.point2.copy(new THREE.Vector3().subVectors(scope.point2,scope.point1).normalize().multiplyScalar(val).add(scope.point1));
       scope.JSQEX_changeSpiralstair()
    } );
    ...
    folderCamera.add( params, '高度值',1, 8000, 1).onChange( function(val) {
...
JSQEXBasicStructure.JSQEX_Stair.prototype.JSQEX_changeSpiralstair = function(json) {
...
JSQEXBasicStructure.JSQEX_Stair_Spiral = function() {
    ...
    this.aEndAngle=.5*Math.PI+0.75*Math.PI*2;
    ...
    this.layout={radius:700,rotate:0.75,width:400};//布局 radius:半径,rotate:旋转,width:宽度
...
JSQEXBasicStructure.JSQEX_Stair_Spiral.prototype.JSQEX_Spiralstair = function(json){
    ...
    for(var i=0,j=arr.length-1;i<j;i++){
        ...
        var ver1=arr[i+1].clone().sub(json.center).multiplyScalar(-1).normalize().multiplyScalar(pdwidth).add(arr[i+1].clone()).add(new THREE.Vector3(0,perheight*(i+1),0)); 
        var ver2=arr[i+1].clone().sub(json.center).normalize().multiplyScalar(pdwidth).add(arr[i+1].clone()).add(new THREE.Vector3(0,perheight*(i+1),0));
        var ver3=arr[i].clone().sub(json.center).multiplyScalar(-1).normalize().multiplyScalar(pdwidth).add(arr[i].clone()).add(new THREE.Vector3(0,perheight*(i+1),0)).add(fxver.clone().normalize().multiplyScalar(overdistance));
        var ver4=arr[i].clone().sub(json.center).normalize().multiplyScalar(pdwidth).add(arr[i].clone()).add(new THREE.Vector3(0,perheight*(i+1),0)).add(fxver.clone().normalize().multiplyScalar(overdistance));
        ...
        var ver_1=ver3.clone().add(_fxver);
        var ver_2=ver_1.clone().add(_normal.clone().multiplyScalar(-1).multiplyScalar(new THREE.Vector3().subVectors(ver2,ver1).dot(_normal.multiplyScalar(-1))));
        ...
        //上面
        var min_max1={point1:ver_1,point2:ver_2,point3:ver_3,point4:ver4};
        var _arr=[
...
JSQEXBasicStructure.JSQEX_Stair_Spiral.prototype.JSQEX_Addsupport_beam = function(json){
    ...
    for(var i=0,j=arr.length-1;i<j;i++){
        ...
        ver_1=arr[i+1].clone().sub(json.center).multiplyScalar(-1).normalize().multiplyScalar(_widthzcl).add(arr[i+1].clone()).add(new THREE.Vector3(0,perheight*(i+1)-tjhd,0)); 
        ver_2=arr[i+1].clone().sub(json.center).normalize().multiplyScalar(_widthzcl).add(arr[i+1].clone()).add(new THREE.Vector3(0,perheight*(i+1)-tjhd,0));
        ver_3=arr[i].clone().sub(json.center).multiplyScalar(-1).normalize().multiplyScalar(_widthzcl).add(arr[i].clone()).add(new THREE.Vector3(0,perheight*(i+1)-tjhd,0));
        ver_4=arr[i].clone().sub(json.center).normalize().multiplyScalar(_widthzcl).add(arr[i].clone()).add(new THREE.Vector3(0,perheight*(i+1)-tjhd,0));
...
JSQEXBasicStructure.JSQEX_Stair_Spiral.prototype.JSQEX_Addside_string = function(json){
    ...
    for(var i=0,j=arr.length-1;i<j;i++){
        ...
        //左边栏上面
        v1=arr[_num].clone().sub(json.center).multiplyScalar(-1).normalize().multiplyScalar(pdwidth+cxwidth).add(arr[_num].clone()).add(new THREE.Vector3(0,perheight*(_num+1),0)).add(_cxpynum);
        v2=arr[_num].clone().sub(json.center).multiplyScalar(-1).normalize().multiplyScalar(pdwidth).add(arr[_num].clone()).add(new THREE.Vector3(0,perheight*(_num+1),0)).add(_cxpynum);
        v3=arr[i].clone().sub(json.center).multiplyScalar(-1).normalize().multiplyScalar(pdwidth+cxwidth).add(arr[i].clone()).add(new THREE.Vector3(0,perheight*_num,0)).add(_cxpynum);
        v4=arr[i].clone().sub(json.center).multiplyScalar(-1).normalize().multiplyScalar(pdwidth).add(arr[i].clone()).add(new THREE.Vector3(0,perheight*_num,0)).add(_cxpynum);
        //右边栏上面
        v5=arr[_num].clone().sub(json.center).normalize().multiplyScalar(pdwidth).add(arr[_num].clone()).add(new THREE.Vector3(0,perheight*(_num+1),0)).add(_cxpynum);
        v6=arr[_num].clone().sub(json.center).normalize().multiplyScalar(pdwidth+cxwidth).add(arr[_num].clone()).add(new THREE.Vector3(0,perheight*(_num+1),0)).add(_cxpynum);
        v7=arr[i].clone().sub(json.center).normalize().multiplyScalar(pdwidth).add(arr[i].clone()).add(new THREE.Vector3(0,perheight*_num,0)).add(_cxpynum);
        v8=arr[i].clone().sub(json.center).normalize().multiplyScalar(pdwidth+cxwidth).add(arr[i].clone()).add(new THREE.Vector3(0,perheight*_num,0)).add(_cxpynum);
        //左边栏下面
...
JSQEXBasicStructure.JSQEX_Stair_Utype = function(a) {
    ...
    this.object_type={beelinetype:false,ltype:false,utype:false,spiraltype:true};
    this.addgeo={side_string:false,support_beam:false};//生成几何体 side_string:侧弦 support_beam：支撑梁   true：添加
    this.layout={length1:1,length2:1,offset:1,width:1};//布局 length1:长度1,length2:长度2,offset:偏移,width:宽度
...
JSQEXBasicStructure.JSQEX_Stair_Utype.prototype.JSQEX_showgui = function(json) {
    ...
    folderCamera.add( params, '长度1',1, 5000, 1).onChange( function(val) {
       ...
       scope.point2.copy(new THREE.Vector3().subVectors(scope.point2,scope.point1).normalize().multiplyScalar(val).add(scope.point1));
       scope.point3.copy(scope.point2);
       scope.point4.copy(new THREE.Vector3().subVectors(scope.point1,scope.point2).normalize().multiplyScalar(scope.layout.length2).add(scope.point3));
       scope.JSQEX_changeSpiralstair()
    } );
    folderCamera.add( params, '长度2',1, 5000, 1).onChange( function(val) {
       scope.layout.length2=val;
       scope.point4.copy(new THREE.Vector3().subVectors(scope.point4,scope.point3).normalize().multiplyScalar(val).add(scope.point3));
       // scope.point3.copy(scope.point2);
       // scope.point4.copy(new THREE.Vector3().subVectors(scope.point4,scope.point3).normalize().multiplyScalar(scope.layout.length2).add(scope.point3));
       scope.JSQEX_changeSpiralstair()
    } );
    folderCamera.add( params, '宽度',0, 2000, 1).onChange( function(val) {
       scope.layout.width=val;
...
JSQEXBasicStructure.JSQEX_Stair_Ltype = function(a) {//L型楼梯对象
    ...
    this.object_type={beelinetype:false,ltype:false,utype:false,spiraltype:true};
    this.addgeo={side_string:false,support_beam:false};//生成几何体 side_string:侧弦 support_beam：支撑梁   true：添加
    this.layout={length1:0,length2:0,offset:1,width:800,angle:90};//布局 length1:长度1,length2:长度2,offset:偏移,width:宽度,angle:角度
    this.step={height:0};//梯级 height:高度
    this.steps={thickness:26,depth:20,subsection:12};//台阶  thickness:厚度,depth:深度,subscction:分段数
    this.support_beam={depth:80,width:30}; //支撑梁 depth:深度,width:宽度
    this.side_string={depth:450,width:20,offset:0}//侧弦 depth:深度,width:宽度,offset:偏移
...
JSQEXBasicStructure.JSQEX_Stair_Ltype.prototype.JSQEX_showgui = function(json) {
    ...
    folderCamera.add( params, '长度1',1, 5000, 1).onChange( function(val) {
       ...
       scope.point2.copy(new THREE.Vector3().subVectors(scope.point2,scope.point1).normalize().multiplyScalar(val).add(scope.point1));
       scope.point3.copy(scope.point2);
       scope.point4.copy(scope.ver1.clone().multiplyScalar(scope.layout.length2).add(scope.point3));
       scope.JSQEX_changeSpiralstair()
    } );
    folderCamera.add( params, '长度2',1, 5000, 1).onChange( function(val) {
       scope.layout.length2=val;
...
JSQEXBasicStructure.JSQEX_Stair_Ltype.prototype.JSQEX_getpoints1 = function(_json){
    ...
    offset=this.layout.offset;
        var arr=[],arr2=[];
        var _points=this.JSQEX_getRotatepoints({point1:point1,point2:point2,angle:angle,width:width,length:length,offset:offset})
        var point3=_points[0];
        var point4=_points[1];
        for (var x = 0; x <= _num+1; x ++ ) {
            if(x<=_num/2){
...
JSQEXBasicStructure.JSQEX_Stair.prototype.JSQEX_getPlanepoints = function(ray,point,height){
   var long1=new THREE.Vector3().subVectors(ray.origin,point).length();
   var long=new THREE.Vector3().subVectors(ray.origin,point).length()*ray.origin.z/(ray.origin.z+height);
   console.log(ray.direction.clone().multiplyScalar(long).add(ray.origin))
   return ray.direction.clone().multiplyScalar(long).add(ray.origin);
}
...
JSQEXBasicStructure.JSQEX_Stair_Beeline.prototype.JSQEX_Addsupport_beam = function(json){
    ...
    for(var i=0,j=Cal_vertice.length;i<j;i++){
        if(Cal_vertice[i][9]!=null){
            ...
            this.JSQEX_changefaceVertexUvs({lx:2,arr:_arr,min_max1:min_maxL,min_max2:min_maxL,materialInd:3});
            //左面
            //右面
            var _arr=[
                [3+ this.JSQEX_verticesnum, 7+ this.JSQEX_verticesnum, 1+ this.JSQEX_verticesnum],
            ...
            this.JSQEX_changefaceVertexUvs({lx:2,arr:_arr,min_max1:min_maxR,min_max2:min_maxR,materialInd:3});
            //右面
            //下面
            var _arr=[
...
JSQEXBasicStructure.JSQEX_Stair_Beeline.prototype.JSQEX_Addside_string = function(json){
    ...
    for(var i=0,j=Cal_vertice.length;i<j;i++){
        ...
        if(Cal_vertice[i][17]!=null){
            ...
            this.JSQEX_changefaceVertexUvs({lx:2,arr:_arr,min_max1:min_maxL1,min_max2:min_maxL1,materialInd:5});
            //左右
            var _arr=[
                [3+ this.JSQEX_verticesnum,11+ this.JSQEX_verticesnum,1+ this.JSQEX_verticesnum],
            ...
            this.JSQEX_changefaceVertexUvs({lx:2,arr:_arr,min_max1:min_maxR1,min_max2:min_maxR1,materialInd:5});
            //右下
            var _arr=[
            ...
            this.JSQEX_changefaceVertexUvs({lx:2,arr:_arr,min_max1:min_maxL2,min_max2:min_maxL2,materialInd:5});
            //右右
            var _arr=[
                [7+ this.JSQEX_verticesnum,15+ this.JSQEX_verticesnum,5+ this.JSQEX_verticesnum],
            ...
            this.JSQEX_changefaceVertexUvs({lx:2,arr:_arr,min_max1:min_maxR2,min_max2:min_maxR2,materialInd:5});
...
JSQEXBasicStructure.JSQEX_Stair_Utype.prototype.JSQEX_Spiralstair = function(json){
    ...
    for(var i=0,j=arr.length-1;i<j;i++){
        ...
        var normal=this.JSQEX_Getnormal(fxver);
        if(i>_pynum){
            _pypos.copy(normal.clone().multiplyScalar(pdwidth*2+_offset)).multiplyScalar(json.pdnum);
        }
        if(i==_pynum){
            if(json.addgeo.support_beam&&json.tjlx.open){//判断是否显示支持梁
    ...
    if(json.pdnum>0){
        ...
        var ver1=arr[i].clone().add(normal.clone().multiplyScalar(_offset+3*pdwidth)).add(new THREE.Vector3(0,perheight*(i+1),0)).add(fxver.clone().normalize().multiplyScalar(-pdwidth*2));
        ...
        var ver3=arr[i].clone().add(normal.clone().multiplyScalar(_offset+3*pdwidth)).add(new THREE.Vector3(0,perheight*(i+1),0));
...
JSQEXBasicStructure.JSQEX_Stair_Utype.prototype.JSQEX_Addsupport_beam = function(json,_arrpt){
    ...
    if(_arrpt!=undefined){
        ...
        var normal=this.JSQEX_Getnormal(fxver);
        var v1=json.arr[0+1].clone().add(normal.clone().multiplyScalar(_widthzcl)).add(new THREE.Vector3(0,perheight*(0+1)-tjhd,0)); 
        var v2=json.arr[0+1].clone().add(normal.clone().multiplyScalar(-_widthzcl)).add(new THREE.Vector3(0,perheight*(0+1)-tjhd,0));
        var v3=json.arr[0].clone().add(normal.clone().multiplyScalar(_widthzcl)).add(new THREE.Vector3(0,perheight*(0+1)-tjhd,0));
        var v4=json.arr[0].clone().add(normal.clone().multiplyScalar(-_widthzcl)).add(new THREE.Vector3(0,perheight*(0+1)-tjhd,0));
    ...
    if(_arrpt==undefined&&json.tjlx.open){
        ...
        var normal=this.JSQEX_Getnormal(fxver);
        var v1=json.arr2[0+1].clone().add(normal.clone().multiplyScalar(_widthzcl)).add(new THREE.Vector3(0,perheight*(0+1)-tjhd,0)).add(new THREE.Vector3(0,_pynum*perheight,0)); 
        var v2=json.arr2[0+1].clone().add(normal.clone().multiplyScalar(-_widthzcl)).add(new THREE.Vector3(0,perheight*(0+1)-tjhd,0)).add(new THREE.Vector3(0,_pynum*perheight,0));
        var v3=json.arr2[0].clone().add(normal.clone().multiplyScalar(_widthzcl)).add(new THREE.Vector3(0,perheight*(0+1)-tjhd,0)).add(new THREE.Vector3(0,_pynum*perheight,0));
        var v4=json.arr2[0].clone().add(normal.clone().multiplyScalar(-_widthzcl)).add(new THREE.Vector3(0,perheight*(0+1)-tjhd,0)).add(new THREE.Vector3(0,_pynum*perheight,0));
        var v5=v1.clone().sub(new THREE.Vector3(0,zclheight,0)); 
        var v6=v2.clone().sub(new THREE.Vector3(0,zclheight,0));
        var v7=v3.clone().sub(new THREE.Vector3(0,perheight+zclheight,0));
        var v8=v4.clone().sub(new THREE.Vector3(0,perheight+zclheight,0));
    ...
    for(var i=0,j=Cal_vertice.length;i<j;i++){
        ...
        if(Cal_vertice[i][9]!=null){
            ...
            this.JSQEX_changefaceVertexUvs({lx:2,arr:_arr,min_max1:min_maxL,min_max2:min_maxL,materialInd:3,uvlx:3});
            //左面
            //右面
            var _arr=[
                [3+ this.JSQEX_verticesnum, 7+ this.JSQEX_verticesnum, 1+ this.JSQEX_verticesnum],
            ...
            this.JSQEX_changefaceVertexUvs({lx:2,arr:_arr,min_max1:min_maxR,min_max2:min_maxR,materialInd:3,uvlx:3});
            //右面
            //下面
            var _arr=[
                [6+this.JSQEX_verticesnum, 8+this.JSQEX_verticesnum,7+this.JSQEX_verticesnum],
...
JSQEXBasicStructure.JSQEX_Stair_Utype.prototype.JSQEX_Addside_string = function(json){
    ...
    for(var i=0,j=Cal_vertice.length;i<j;i++){
        ...
        if(Cal_vertice[i][19]!=null){
            ...
            this.JSQEX_changefaceVertexUvs({lx:2,arr:_arr,min_max1:min_maxL1,min_max2:min_maxL1,materialInd:5,uvlx:3});
            //左右
            var _arr=[
                [3+ this.JSQEX_verticesnum,11+ this.JSQEX_verticesnum,1+ this.JSQEX_verticesnum],
                [11+ this.JSQEX_verticesnum,17+ this.JSQEX_verticesnum,1+ this.JSQEX_verticesnum],
            ...
            this.JSQEX_changefaceVertexUvs({lx:2,arr:_arr,min_max1:min_maxR1,min_max2:min_maxR1,materialInd:5,uvlx:3});
            //右下
            var _arr=[
                [14+ this.JSQEX_verticesnum,18+ this.JSQEX_verticesnum,15+ this.JSQEX_verticesnum],
                [18+ this.JSQEX_verticesnum,19+ this.JSQEX_verticesnum,15+ this.JSQEX_verticesnum],
            ...
            this.JSQEX_changefaceVertexUvs({lx:2,arr:_arr,min_max1:min_maxL2,min_max2:min_maxL2,materialInd:5,uvlx:3});
            //右右
            var _arr=[
                [7+ this.JSQEX_verticesnum,15+ this.JSQEX_verticesnum,5+ this.JSQEX_verticesnum],
                [15+ this.JSQEX_verticesnum,19+ this.JSQEX_verticesnum,5+ this.JSQEX_verticesnum],
            ...
            this.JSQEX_changefaceVertexUvs({lx:2,arr:_arr,min_max1:min_maxR2,min_max2:min_maxR2,materialInd:5,uvlx:3});
            this.JSQEX_verticesnum+=20;
        }else{
            //左下
            var _arr=[
